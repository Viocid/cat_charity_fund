...
def don_char(donations, projects, session):
    ...
    while donation_index < len_donations and project_index < len_projects:
        first_donation = donations[donation_index]
        first_project = projects[project_index]
        donation_capacity = (
            first_donation.full_amount - first_donation.invested_amount
        )
        project_capacity = (
            first_project.full_amount - first_project.invested_amount
        )
        ...
        if first_donation.invested_amount == first_donation.full_amount:
            first_donation.fully_invested = True
            first_donation.close_date = datetime.now()
            donation_index += ONE

        if first_project.invested_amount == first_project.full_amount:
            first_project.fully_invested = True
            first_project.close_date = datetime.now()
            project_index += ONE
        first_donation = donation_crud.update_investment(
            session, first_donation
        )
        first_project = charity_project_crud.update_investment(
...
async def investments(
    ...
    try:
        ...
        projects = await charity_project_crud.get_active_order_by_create_date(
            session
        )
        if not (donations or projects):
            return
        don_char(donations, projects, session)
        await donation_crud.investment(session, new_db_obj)

        return new_db_obj

    except SQLAlchemyError as error:
        await donation_crud.roll(session)
